...
const PaginationNavigation: React.FC<PaginationNavigationProps> = ({
    ...
    return (
        <div
            className={`flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 pt-6 sm:pt-8 mt-6 sm:mt-8 border-t border-gray-200 dark:border-gray-800 ${className}`}
        >
            {/* Previous Page */}
            <div>
                {previousPage && (
                    <Link to={previousPage.href}>
                        <Button
                            variant="ghost"
                            className="w-full sm:w-auto flex items-center gap-2 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800 justify-between sm:justify-start px-4 py-3"
                        >
